...
/**
 * SafeWalletButton
 * Renders a friendly state while wallet adapters initialize to avoid WalletNotReadyError.
 */
function SafeWalletButton() {
  ...
  const ready =
    ...
    wallet?.adapter?.readyState === WalletReadyState.Loaded;

  if (!wallets || wallets.length === 0) {
    return (
      <button
        disabled
        className="px-4 py-2 rounded-full bg-white/5 text-xs font-bold text-zinc-400"
        title="No wallet adapters available"
      >
        Wallet Unavailable
      </button>
    );
  ...
  if (!ready) {
    return (
      <button
        disabled
        className="px-4 py-2 rounded-full bg-white/5 text-xs font-bold text-zinc-400"
        title="Waiting for wallet extension to initialize"
      >
        Waiting for Wallet…
      </button>
    );
...
export default function MerchantAdmin() {
  ...
  useEffect(() => {
    try {
      if (pairingToken) localStorage.setItem("active_pairing_code", pairingToken);
      else localStorage.removeItem("active_pairing_code");
    } catch {}
  }, [pairingToken]);
