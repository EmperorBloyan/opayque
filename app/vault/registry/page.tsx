...
export default function RegistryPage() {
  ...
  useEffect(() => {
    ...
    const validateVaultContext = async () => {
      ...
      // If still no merchantId, send the user through onboarding instead of exposing stale vault state.
      if (!merchantId || merchantId === "merchant-vault") {
        console.warn("Invalid vault context. Redirecting to onboarding.");
        clearActiveSession();
        if (typeof window !== "undefined") {
          window.localStorage.setItem("opayque_next_route", "/vault/registry");
          window.localStorage.removeItem("opayque_tx");
          window.localStorage.removeItem("opayque_balance");
        }
