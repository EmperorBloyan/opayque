...
export default function ApiKeysPage() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      // 2. Fetch Sync from Backend API + Supabase
      try {
        ...
        // Also load secret key from merchants table directly
        if (user) {
          ...
          if (merchantData?.api_key && keyPairs.length === 0) {
            // Create a display key from saved secret
  ...
  const handleCreateKey = async () => {
    ...
    try {
      ...
      // CRITICAL: Save to merchants table so it persists
      if (newKey?.secret) {
  ...
  const handleSignOut = async () => {
    try {
      await supabase.auth.signOut(); // <-- CHANGED to use existing client
    } catch (error) {
  ...
  return (
    <main className="min-h-screen bg-zinc-950 p-6 md:p-12 font-sans relative overflow-hidden text-white">
      ...
      <div className="max-w-7xl mx-auto space-y-10">
        ...
        {/* Paste everything from your original return() down here */}
