...
describe("merchant wallet resolution", () => {
  ...
  it("prefers a dedicated refund wallet and otherwise falls back through settlement to legacy wallet", () => {
    ...
    expect(resolveRefundWallet({ wallet_address: "legacy" })).toEqual({ address: "legacy", source: "legacy" });
  });
});
